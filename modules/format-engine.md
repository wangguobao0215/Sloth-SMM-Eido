...
| 参数 | 可选值 | 说明 |
|------|-------|------|
| `theme` | `default` / `tech` / `business` / `minimal` / `elegant` | 整体视觉风格，默认 `default` |
| `color` | 任意 Hex 色值，如 `#1AAD19` / `#FF6B6B` / `#4A90D9` | 主题强调色，用于引用块、分割线、小标题装饰 |
| `font_size` | `14px` / `15px` / `16px` / `17px` | 正文字号，默认 `16px` |
| `line_height` | `1.6` / `1.8` / `2.0` | 行高倍数，默认 `1.8` |
| `max_width` | `680px` / `720px` / `800px` | 内容区最大宽度，默认 `680px` |
...
**AI 配图触发条件**：
1. **强制触发**：所有长文平台版本（公众号/头条/知乎）在发布前必须生成配图
2. **风格匹配触发**：根据母版文章风格自动选择配图 preset
   - `deep`（深度干货）→ 使用 `edu-visual` 或 `blueprint`（信息图/蓝图风格）
   - `story`（故事情感）→ 使用 `warm-illustration` 或 `watercolor`（温暖插画/水彩风格）
   - `sharp`（观点犀利）→ 使用 `editorial` 或 `poster`（社论/海报风格）
   - `light`（轻松科普）→ 使用 `minimal-flat` 或 `notion`（极简扁平风格）
3. **用户覆盖触发**：若 EXTEND.md 中配置了 `illustration.style`，优先使用用户指定风格
4. **跳过条件**：仅在用户明确指令"不配图"或"纯文字"时可跳过，需在输出中标注 `[无配图]`
...
| 规则项 | 说明 |
|--------|------|
| **图卡数量** | 通常 4-8 张，封面图 + 内容卡 + 结尾图 |
| **封面图** | 第 1 张，必须包含标题核心信息和强视觉吸引点，3:4 竖版比例 |
| **内容拆分** | 按 xiaohongshu.md 中的要点逐条拆分为独立卡片，每张卡片 1-2 个要点 |
| **视觉一致性** | 全系列图卡使用统一的配色方案、字体风格和装饰元素（Reference Image Chain） |
| **结尾图** | 最后 1 张，引导关注/收藏/评论，可包含账号昵称和头像占位 |
| **文字限制** | 单张卡片文字不超过 80 字，字号足够大，确保手机小屏可读 |
| **emoji 使用** | 图卡中适度使用 emoji 做视觉分隔，与文案风格保持一致 |
| **安全边距** | 四周预留 10% 安全边距，避免被平台裁切 |
